...
class FullAPRModel(nn.Module):
    def __init__(self, model_name, vocab_size, pad_token_id, start_token_id):
        super().__init__()
    ...
    def forward(self, input_values, labels=None, attention_mask=None):
        ...
        if labels is not None:
            ...
            decoder_inputs = labels.clone()
            decoder_inputs[~labels_mask] = self.pad_token_id
            
            start_tokens = torch.full((labels.size(0), 1), self.start_token_id, dtype=torch.long, device=labels.device)
            decoder_inputs = torch.cat([start_tokens, decoder_inputs], dim=1)[:, :-1]
            ...
            att_loss = self.att_loss_fct(att_logits.reshape(-1, att_logits.size(-1)), labels.view(-1))
